...
var TabUtil = {
    ...
    find_all: function (query_obj, callback_ok, callback_fail, do_unmerge) {
        ...
        return new Promise(function (resolve, reject) {
            try {
                var class_config = TablesMeta.get_class_config(query_obj.className);
                if (class_config.support_merge_fields == true) {
                    is_merged = true;
                    query_obj.limit(G.get_limit_for_merged());
                }
                console.log("class_name: " + query_obj.className + ", is_merged: " + is_merged);
                var limit = query_obj._limit > 0 ? query_obj._limit : is_merged ? G.get_limit_for_merged() : G.TAB_LIMIT;
                    ...
                    TabUtil.find(query_obj, function (records) {
                        try{
                            ...
                            if (records.length >= limit){
                                ...
                                if (do_unmerge) {
                                    console.log("find_all.query_before: do_merge start");
                                    record_list = TabUtil.try_unmerge(record_list);
                                    console.log("find_all.query_before: do_merge start 1");
                                    record_list = TabUtil.try_sort_after_unmerge(query_obj, record_list);
                                    console.log("find_all.query_before: do_merge end");
                                }
                                if (resolve)
                                    resolve(record_list);
                                if (callback_ok)
    ...
    copy_record: function (record) {
        ...
        for (var key in record.attributes) {
            if (! record.attributes.hasOwnProperty(key) || key == TabUtil._merged_field)
                continue;
    ...
    unmerge_records: function (records, merged_field) {
        ...
        try {
            records.forEach(function (x) {
                ...
                merge_list.forEach(function (record) {
                    ...
                    for (var key in record) {
                        if (! record.hasOwnProperty(key))
                            continue;
                        new_record.set(key, record[key]);
                    }
                    new_records.push(new_record);
                });
                console.log('unmerge_records memory: ' + util.inspect(process.memoryUsage()));
            });
            return new_records;
        } catch (e) {
            throw e;
    ...
    save_records_with_merge: function (records, fields_to_merge, fields_common, merged_field, max_merged_num) {
        ...
        if (fields_to_merge == undefined) {
            return new Promise(function(resolve, reject) {
                reject(new Error("not get fields_to_merge, do you use the right cgi('/service/SaveRecordWithCache') or class-name ?"))
            });
        }
