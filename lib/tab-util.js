...
var TabUtil = {
    ...
    record_recover_merge: function (records, merge_field, when_ok, when_fail) {
        var new_records = [];
        new_records.forEach(function (x) {
            var merge_list = x.get(merge_field);
            if (! merge_list) {
                new_records.push(x);
                return;
            }
            if (typeof (merge_list) != 'object') {
                var e = new Error("typeof (merge_field: " + merge_field + ") must be list !");
                console.error(e);
                if (when_fail) {
                    when_fail(e);
                }
            ...
            merge_list.forEach(function (x) {
                if (typeof (x) != 'object') {
                    var e = new Error("typeof (item of: " + merge_field + ") must be object !");
                    console.error(e);
                    when_fail(e);
                }
