...
var TabUtil = {
    ...
    find: function (query_obj, callback_ok, callback_fail, try_count) {
        ...
        return new Promise(function (resolve, reject) {
            query_obj.find().then(function (records) {
                if (records.length > 0 && (TabUtil._merged_field in records[0].attributes)) {
    ...
    save_records_with_merge: function (records, fields_to_merge, merged_field, max_merged_num) {
        var merged_list = [];
        max_merged_num = max_merged_num || TabUtil._max_merged_num;

        if (!fields_to_merge || fields_to_merge.length == 0) {
            return new Promise(function(resolve, reject) {
        ...
        var first_record = records[0];
        var query_last_merge = new AV.Query(first_record.className);
        var record_list = [];
